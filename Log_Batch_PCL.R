...
pcl_pathways_df <- pcl_df %>% filter(omni_comparison == "comparison") %>%
  select(study_id, participant_id, collection_type, pathways)
...
pcl_pathways_df <- pcl_pathways_df %>%
  gather("pathway", "val", -c(study_id, participant_id, collection_type))
...
pcl_pathways_df2 %>% ggplot(aes(x = study_id, y = mean_val, fill = pathway)) +
...
pcl_pathways_df2 <- pcl_pathways_df %>%
  ...
  summarize(mean_val = mean(val)) %>%
  arrange(mean_val)
pdf(file.path(graph_dir, "Pathways_Aggregated.pdf"), width = 18, height = 12)
pcl_pathways_df2 %>% ggplot(aes(x = collection_type, y = mean_val, fill = pathway)) +
  geom_bar(position = "dodge", stat = "identity") +
