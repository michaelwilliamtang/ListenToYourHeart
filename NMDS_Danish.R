...
nmds_danish <- function(ds1) {
  ...
  dev.off()
  print("NMDS")
  nmds <- metaMDS(met_data, distance = "bray", k = 3, trymax = 500, na.rm = T, autotransform = F)
  ...
  gg <- ggplot(nmds_scores, aes(x = NMDS1, y = NMDS3, fill = TRIMESTER, color = TRIMESTER)) +
    geom_point(size = 3) +
    labs(title = desc)
  plot(gg)
  ggsave(file.path(graph_dir2, paste(ds1, "NMDS1-3.pdf", sep = "_")), plot = last_plot())

  gg <- ggplot(nmds_scores, aes(x = NMDS2, y = NMDS3, fill = TRIMESTER, color = TRIMESTER)) +
    geom_point(size = 3) +
    labs(title = desc)
  plot(gg)
