# 6/12/20
# Create NMDS graph for batch omni vs unbatch omni vs batch reg vs omni reg PCL
...
nmds_batch_unbatch_omni_reg_comp <- function(pcl_lab, sing_lab, N) {
  ...
  gg <- ggplot(nmds_scores, aes(x = NMDS1, y = NMDS2, fill = participant_id, color = participant_id, shape = collection_type)) + 
    geom_point(aes(size = batched)) +  # Set color to vary based on subject
...
### comparing for pcl
nmds_batch_unbatch_omni_reg_comp(pcl_lab = "Pathways", sing_lab = "Pathway", N = 40)