# 8/30/20
# Filter and plot analyte-metadatum pairs from Danish data, split into tertiles
...
danish_corr_inv <- function(comp_N = NA) {
  ...
  graph_dir2 <- file.path(graph_dir, paste(ds1, "Corr_Tertiles_Top", comp_N, sep = "_"))
  ...
  for (i in 1:length(analytes)) {
    ...
    dci_metadatum(corr_df, tidy_df, tidy_metadata, clean_names[i], meta1, analytes2[i], graph_dir3, comp_N)
  }
...
dci_metadatum <- function(corr_df, data_df, metadata_df, cname, meta1, meta2, graph_dir3, comp_N) {
  ...
  for (sel in diff_N_anl) {
    tryCatch({
      ...
      # plot
      desc <- paste0(cname, " vs ", sel, ", Tertiles")
      print(desc)
      gg <- ggplot(comb_sel, aes(x = tertile, y = val, fill = tertile)) +
        geom_violin(trim = F) +
        ...
        xlab(cname) +
        ylab(sel) +
        labs(title = desc, color = "Participant") +
        guides(fill = F)
