# 12/15/20
# Graph participant correlations from new iPOP data, extract missing and mismatch info

library(tidyverse)

ncmr_investigate <- function(source_dir) {
  print(source_dir)
  
  # get locs
  data_dir <- file.path("Data", source_dir)
  graph_dir <- file.path("Graphs", "RNA_Seq", source_dir)
  ...
  guide <- read.table(file.path(data_dir, "Batch_6_metadata.txt"), sep= "\t", header = 1, stringsAsFactors = F)
  # id_index <- guide$sample.name %>% str_locate("_") - 1 # dynamically finds any length id
  # guide$id <- guide$sample.name %>% substr(0, id_index)
  sample_num <- regmatches(as.character(guide$Sequencing.Sample), gregexpr("[[:digit:]]+", as.character(guide$Sequencing.Sample))) %>%
    unlist()
  guide$new_full_name <- paste0("Fiber_68s_", sample_num, "_S", sample_num, "Aligned.sortedByCoord.out.vcf")
  ...
  write.csv(data$V[missing_rows], file.path(graph_dir, "Missing.csv"))
  
  # match data with guide
  samples <- guide$sample.name
  names(samples) <- guide$new_full_name
  ids <- as.character(guide$Participant)
...
ncmr_investigate("iPOP_December")