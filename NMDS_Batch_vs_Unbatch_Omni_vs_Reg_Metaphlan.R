# 6/12/20
# Create NMDS graph for batch omni vs unbatch omni vs batch reg vs omni reg metaphlan
...
ds1 <- "metaphlan"
set.seed(10)
desc <- "Omnigene vs Regular and Batch Corrected vs Uncorrected,\nNMDS (Bray-Curtis Distance)"
# library(icesTAF)
# library(metaMA)
# library(lme4)
# library(gtools)
...
nmds_batch_unbatch_omni_reg_comp <- function(met_lev, met_reg, met_lab, sing_lab, N) {
  print(met_lab)
  ...
  met_metadata <- met_phyla_df %>%
    select(participant_id, collection_type, batched)
...
nmds_batch_unbatch_omni_reg_comp(met_lev = "phylum", met_reg = ".*p__", met_lab = "Phyla", sing_lab = "Phylum", N = 5)

### comparing for metaphlan, genera
nmds_batch_unbatch_omni_reg_comp(met_lev = "genus", met_reg = ".*g__", met_lab = "Genera", sing_lab = "Genus", N = 20)